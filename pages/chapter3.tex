...
\textbf{（2）优化目标：}
PPO的优化目标基于$\hat{A}(obs, action)$。当$\hat{A}(obs, action) > 0$时，表示当前策略比期望策略更好，
需要增加当前策略的概率；当$\hat{A}(obs, action) < 0$时，表示当前策略比期望策略更差，
需要减少当前策略的概率。
...
\[
L^{\mathrm{CLIP}}(\theta)
= \mathbb{E}{(s,a,r) \sim \mathcal{D}} \Big[ \min\big( r(\theta)\, \hat{A},\;
\operatorname{clip}\big( r(\theta),\, 1-\epsilon,\, 1+\epsilon \big)\, \hat{A} \big) \Big]
\]

其中$D$为多局单步游戏收集的$(obs, aciton, reward)$组成的集合，
$\epsilon$为裁剪半径，裁剪函数$clip(r(\theta),1-\epsilon,1+\epsilon)$
将概率比限制在$[1-\epsilon,1+\epsilon]$，以截断更新幅度。

策略更新的目标为最大化$L^{CLIP}(\theta)$，
或最小化$-L^{CLIP}(\theta)$
